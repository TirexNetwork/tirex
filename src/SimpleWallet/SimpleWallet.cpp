...
bool simple_wallet::new_wallet(const std::string &wallet_file, const std::string& password) {
  ...
  try {
    ...
    logger(INFO, BRIGHT_GREEN) << "TirexWallet is an open-source, client-side, free wallet which allow you to send and receive TRX instantly on the blockchain. You are  in control of your funds & your keys. When you generate a new wallet, login, send, receive or deposit $TRX everything happens locally. Your seed is never transmitted, received or stored. That's why its imperative to write, print or save your seed somewhere safe. The backup of keys is your responsibility. If you lose your seed, your account can not be recovered. The Conceal Team doesn't take any responsibility for lost funds due to nonexistent/missing/lost private keys." << std::endl << std::endl;
...
bool simple_wallet::get_reserve_proof(const std::vector<std::string> &args)
{
	...
	try {
		...
		const std::string filename = "reserve_proof_" + args[0] + "_TRX.txt";
...
bool simple_wallet::export_keys(const std::vector<std::string>& args/* = std::vector<std::string>()*/) {
  ...
  logger(INFO, BRIGHT_GREEN) << std::endl << "TirexWallet is an open-source, client-side, free wallet which allow you to send and receive TRX instantly on the blockchain. You are  in control of your funds & your keys. When you generate a new wallet, login, send, receive or deposit $TRX everything happens locally. Your seed is never transmitted, received or stored. That's why its imperative to write, print or save your seed somewhere safe. The backup of keys is your responsibility. If you lose your seed, your account can not be recovered. The Conceal Team doesn't take any responsibility for lost funds due to nonexistent/missing/lost private keys." << std::endl << std::endl;
